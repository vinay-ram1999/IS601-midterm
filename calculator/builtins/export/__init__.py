...
class ExportOperation(BuiltInOperation):
    def __init__(self) -> None:
        kwargs = {'description':'Exports the history into a .csv file.',
                  'arguments': '1',
    ...
    def execute(self, *args, **kwargs):
        ...
        data_dir = self.env_vars['DATA_DIR']; fname = f'{args[0][0]}.csv'; fpath = os.path.join(data_dir,fname)
        print(os.path.abspath(fpath))
        print(f"Exported the history to {fpath}!"); logging.info(f"Exported the history to {fpath}"); return fpath
